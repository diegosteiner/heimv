...
export const PublicBookingForm = ({ booking, homes = null }) => {
  ...
  const { register, handleSubmit, errors, control, getValues } = useForm({ mode: 'onChange' })
  ...
  return (
    <Form noValidate onSubmit={handleSubmit(onSubmit)}>
      ...
      <Form.Group>
        <Form.Check type="checkbox">
          <Form.Check.Label className={!!errors.accept_conditions && 'is-invalid'}>
            <Form.Check.Input type="checkbox" isInvalid={!!errors.accept_conditions} name="accept_conditions" ref={register({ required: true })}></Form.Check.Input>
            <Trans
      ...
      <Button className="mt-4" type="submit" size='lg'>{t('public.bookings.new.submit')}</Button>
    </Form>
  )
}