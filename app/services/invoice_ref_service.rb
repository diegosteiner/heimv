...
class InvoiceRefService
  ...
  DEFAULT_TEMPLATE = '%<prefix>s%<home_id>03d%<tenant_id>06d%<invoice_id>07d'

  def initialize(organisation)
    @organisation = organisation
  end

  def generate(invoice, template_string = @organisation.invoice_ref_template)
    with_checksum format(template_string.presence || DEFAULT_TEMPLATE,
                         prefix: digits(invoice.organisation.esr_ref_prefix).join,
                         home_id: invoice.booking.home_id,
                         tenant_id: invoice.booking.tenant_id,
