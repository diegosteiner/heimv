  ...
  class Factory
    def for_booking(booking, invoice, invoice_parts = invoice.invoice_parts)
      [from_deposit(invoice)] +
        booking.usages.where.not(id: invoice_parts.map(&:usage_id)).map { |usage| from_usage(invoice, usage) }
    end
    ...
    def label_2(usage)
      ...
      [
        usage.tarif.unit &&
          format('%<used_units>f %<unit>s', used_units: usage.used_units, unit: usage.tarif.unit),
    ...
    def from_deposit(invoice, deposits = invoice.booking.invoices.deposit)
      InvoiceParts::Add.new(
        apply: invoice.invoice_type == :invoice,
