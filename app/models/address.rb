...
class Address
  ...
  def complete?
    attributes.slice(*%w[recipient street postalcode city country_code]).values.all?(&:present?)
  end

  def blank?
    attributes.slice(*%w[recipient street postalcode city]).values.all?(&:blank?)
  end
