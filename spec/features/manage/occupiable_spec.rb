...
describe 'Occupiable', :devise do
  ...
  it 'can create new occupiable' do
    ...
    submit_form
    expect(page).to have_text I18n.t('flash.actions.create.notice', resource_name: Occupiable.model_name.human)
    expect(page).to have_text new_occupiable.name
  end

  it 'can see a occupiable' do
    occupiable
    visit manage_occupiables_path
    expect(page).to have_text occupiable.name
  end
  ...
  it 'can edit existing occupiable' do
    ...
    fill_in :occupiable_name_de, with: new_occupiable.name
    submit_form
    expect(page).to have_text I18n.t('flash.actions.update.notice', resource_name: Occupiable.model_name.human)
    expect(page).to have_text new_occupiable.name
  end
  ...
  it 'can delete existing occupiable' do
    ...
    expect(page).to have_text I18n.t('flash.actions.destroy.notice', resource_name: Occupiable.model_name.human)
  end
end
