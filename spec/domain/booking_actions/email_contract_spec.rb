...
describe BookingActions::EmailContract do
  ...
  describe '#invokable_with' do
    ...
    it { expect(invokable_with[:label]).to eq(I18n.t('booking_actions.email_contract.label_contract_only')) }

    context 'with deposit' do
      before { Invoice::Factory.new(booking).build(type: Invoices::Deposit.to_s).tap(&:save) }

      it { expect(invokable_with[:label]).to eq(I18n.t('booking_actions.email_contract.label_with_deposits')) }
    end
  end
end
